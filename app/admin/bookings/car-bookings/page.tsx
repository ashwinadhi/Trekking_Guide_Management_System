...
import { Car, Loader2, Search, CheckCircle, AlertCircle, Download } from "lucide-react";
...
export default function CarBookingsPage() {
    ...
    try {
      ...
      if (res.ok) {
        ...
        if (searchQuery) {
          ...
          data = data.filter((b: any) => b.customerName?.toLowerCase().includes(q) || b.vehicleName?.toLowerCase().includes(q));
        }
        setBookings(data);
      }
  ...
  const updateStatus = async (id: string, status: string) => {
    setActionLoading(id);
    try {
      const res = await fetch(`/api/vehicle-bookings/${id}`, { method: "PUT", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ status }) });
  ...
  const exportCSV = () => {
    const rows = bookings.map(b => [b.customerName, b.vehicleName, b.status, b.totalPrice, b.startDate]);
    const csv = ["Customer,Vehicle,Status,Price,Date", ...rows.map(r => r.join(","))].join("\n");
    const blob = new Blob([csv], { type: "text/csv" }); const a = document.createElement("a"); a.href = URL.createObjectURL(blob); a.download = "car-bookings.csv"; a.click();
  };
  ...
  return (
    <div className="max-w-7xl mx-auto space-y-6">
      <div className="flex items-center justify-between">
        <div><h1 className="text-3xl font-bold bg-gradient-to-r from-emerald-400 to-teal-400 bg-clip-text text-transparent">Car Bookings</h1><p className="text-gray-400 mt-1">Manage vehicle booking requests</p></div>
      ...
      <div className="bg-gradient-to-br from-gray-800/90 to-gray-900/90 rounded-2xl border border-gray-700/50 overflow-hidden">
        <div className="p-4 border-b border-gray-700/50">
          <div className="relative max-w-md"><Search className="absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 text-gray-500" /><input type="text" value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} placeholder="Search..." className="w-full pl-10 pr-4 py-2 bg-gray-900/50 border border-gray-700 rounded-lg text-white text-sm focus:outline-none focus:ring-2 focus:ring-emerald-500/50" /></div>
        </div>
        {bookings.length === 0 ? (<div className="p-12 text-center"><Car className="h-16 w-16 text-gray-600 mx-auto mb-4" /><h3 className="text-lg font-semibold text-gray-300">No car bookings</h3></div>) : (
